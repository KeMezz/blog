...
예시로 살펴보자면 사용자가 실수 등으로 인풋에 공백이 삽입된 경우, 모든 공백을 없애주는 역할을 하는 함수가 `trim()`이다. 그리고 이메일의 첫 문자를 대문자로 보내는 경우도 있을 수 있는데, 이 때 이메일 형식을 노멀라이즈 해주는 함수인 `normalizeEmail()`도 활용할 수 있다.
